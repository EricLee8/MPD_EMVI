...
class MRCModel(pretrained_model_class):
    def __init__(self, config):
        ...
        self.key_utterance_classifier = nn.Linear(config.hidden_size*4, 1)
    ...
    def forward(
        ...
        indicator_ids = ids_dict['indicator_ids']
        p_mask = ids_dict['p_mask']
        utterance_p_mask = ids_dict['utterance_p_mask']
        key_utterance_target = ids_dict['key_utterance_target']
        ...
        key_utterance_loss_fct = CrossEntropyLoss()

        # compute gate logits
        pooler_output = transformer_outputs.pooler_output if args.model_type=='bert' else hidden_states[:, self.cls_index, :]
        gate_logits = self.verifier(pooler_output).squeeze(-1) # (bsz)
        
        # get utterance_embedding
        utter_embs, question_embs, batch_utter_nums = [], [], []
        for bidx in range(bsz):
            cur_all_utter_embs = []
            for s, e in sep_poses[bidx]:
               cur_all_utter_embs.append(hidden_states[bidx, s: e, :].mean(dim=0)) # (hsz)
            cur_utter_num = len(cur_all_utter_embs) - 1 # the last utterance is the question
            ...
            question_embs.append(cur_all_utter_embs[-1]) # (hsz,)

        # compute key-utterance logits
        utter_embs = torch.stack(utter_embs, dim=0) # (bsz, max_utter, hsz)
        question_embs = torch.stack(question_embs, dim=0).unsqueeze(1).expand_as(utter_embs) # (bsz, max_utter, hsz)
        key_utter_logits = self.key_utterance_classifier(torch.cat([utter_embs, question_embs,\
             utter_embs-question_embs, utter_embs*question_embs], dim=-1)).squeeze(-1) # (bsz, max_utter)
        key_utter_logits = key_utter_logits * utterance_p_mask - 1e30 * (1-utterance_p_mask)
        if training:
            key_utter_loss = key_utterance_loss_fct(key_utter_logits, key_utterance_target) # ignore -100
        else: # modify indicator_ids based on key-utterance predictions
            key_utter_pred = torch.argmax(key_utter_logits, dim=-1) # (bsz)
            for bidx in range(bsz):
                s, e = sep_poses[bidx][key_utter_pred[bidx].item()]
        ...
        hidden_states = transformer_outputs.last_hidden_state # (bsz, seqlen, hsz)
        ...
        if training:
            ...
            total_loss = span_loss + gate_loss*0.5 + key_utter_loss
            loss_dict = {"SL": start_loss.item(), "EL": end_loss.item(), "GL": gate_loss.item(), "UL": key_utter_loss.item()}
        ...
        outputs = (total_loss, loss_dict) if training else (answer_list, na_list,)
        return outputs
