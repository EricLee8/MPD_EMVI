...
def convert_examples_to_features(examples, tokenizer, max_length, training=True, max_utterance_num=args.max_utter_num):
    ...
    def _get_key_utterance_target(start_pos, end_pos, sep_poses):
        for idx, (s, e) in enumerate(sep_poses):
            if s <= start_pos <= end_pos <= e:
                return idx
        return -100
    ...
    total_num, unptr_num, unptr_utter_num, too_long_num = len(examples), 0, 0, 0
    features = []
    for exp in tqdm(examples, ncols=100):
        ...
        utterance_p_mask = [1] * len(exp.utterances) + [0] * (max_utterance_num-len(exp.utterances))
        utterance_p_mask = utterance_p_mask[:max_utterance_num]
        ...
        ids_dict['utterance_p_mask'] = utterance_p_mask
        ...
        if not training:
            ...
            ids_dict.update({'indicator_ids': indicator_ids})
        ...
        key_utterance_target = _get_key_utterance_target(start_pos, end_pos, sep_poses) if not is_impossible else -100
        if not is_impossible and key_utterance_target == -100:
            unptr_utter_num += 1
        ids_dict.update({'key_utterance_target': key_utterance_target})
        if key_utterance_target != -100:
            s, e = sep_poses[key_utterance_target]
    ...
    print("Unpointable key-utterance num: %d, unpointable rate %.2f%%" %(unptr_utter_num, unptr_utter_num/total_num * 100))
